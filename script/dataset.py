...
class SplitDataset:
    def __init__(
        ...
        self._data_train, self._data_test = pd.DataFrame(data[~split]), pd.DataFrame(data[split])
        self._labels_train, self._labels_test = pd.Series(labels[~split]), pd.Series(labels[split])
    ...
    def get_train(self) -> list[DataSet]:
        return [
            (lambda idx_train:
                 (self._data_train.iloc[idx_train], self._labels_train.iloc[idx_train])
            )(self._folds[fold][0])
